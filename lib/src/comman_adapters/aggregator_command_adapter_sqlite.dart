...
mixin AggregatorCommandAdapterSqlite {
  static SqliteCommandPackage convertToPackage({required AggregatorCommand command}) {
    ...
    buffer.write(') VALUES (');
    buffer.write(TextUtilities.generateCommand(list: command.dataField.keys, function: (p0) => '?'));
    buffer.write(');');

    return SqliteCommandPackage(commandText: buffer.toString(), shieldedValues: command.dataField.values.toList());
  }
}
